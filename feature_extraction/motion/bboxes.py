"""Detect object bounding boxes and save them as per-frame arrays."""
...
def delete_overlapped_bboxes(bboxes, bbox_areas, cover_threshold, classes):
    """Drop the smaller box of any pair whose overlap ratio exceeds cover_threshold."""
...
def get_foreground_bboxes(img_batch, bboxes, area_threshold, binary_threshold, gauss_mask_size):
    ...
    sum_grad = cv2.threshold(sum_grad, binary_threshold, 255, cv2.THRESH_BINARY)[1]
...
def extract(dataset_root, dataset_name, train, weights_path=None):
    ...
    predictor = Predictor(confidence_threshold=confidence_threshold, weights_path=weights_path)
    all_bboxes = []
    all_classes = []
    ...
    all_classes = np.array(all_classes, dtype=object)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--weights_path", type=str, default=None,
                        help='path to Mask R-CNN weights (default: ./checkpoints/model_final_f10217.pkl)')
    args = parser.parse_args()
    root = os.path.abspath(args.root)
    extract(dataset_root=root, dataset_name=args.dataset, train=args.train,
