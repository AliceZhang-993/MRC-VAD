...
def get_foreground(img, bboxes, patch_size):
    """Crop every bounding box and resize it to patch_size."""
    img_patches = []
    if len(img.shape) == 3:
        ...
        for i in range(len(bboxes)):
            ...
            img_patches.append(np.array(tmp_set))
        img_patches = np.array(img_patches)
    return img_patches
...
class VideoDataset(Dataset):
    ...
    def get_gt(self):
        if self.dataset_name == 'ped2':
            ...
            for i, (_, video) in enumerate(self.videos.items()):
                ...
                sub_video_gt = np.zeros((length,), dtype=np.int8)
                one_abnormal = abnormal_mat[i]
                if isinstance(one_abnormal, np.ndarray) and one_abnormal.size == 0:
                    self.all_gt.append(torch.from_numpy(sub_video_gt))
                    continue
                if one_abnormal.ndim == 1:
                    one_abnormal = one_abnormal.reshape((one_abnormal.shape[0], -1))
                for j in range(one_abnormal.shape[1]):
    ...
    def get_sequence(self, index):
        """Return the frame indices of the clip centred on / ending at index."""
        ...
        if all_frames_same_video[0] != 0 and all_frames_same_video[-1] != 0:
    ...
    def __getitem__(self, index):
        ...
        if self.normalize:
            img_batch = img_batch.to(dtype=torch.get_default_dtype()).div(255)
...
class VideoDatasetWithFlows(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.all_bboxes is not None:
            ...
            img_batch, flows_batch = self.resize_batch(frame_idx_range, img_batch, flows_batch)
        img_batch = torch.from_numpy(img_batch)
        flows_batch = torch.from_numpy(flows_batch)
